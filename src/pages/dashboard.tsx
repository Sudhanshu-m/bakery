...
import { Users, Cake, Heart, Send, CheckCircle2, Clock, Wifi, WifiOff } from "lucide-react";
import { BarChart, Bar, XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer } from "recharts";
import { AppLayout } from "@/components/layout/app-layout";
import { Badge } from "@/components/ui/badge";
import { getDashboardStats, getCustomers, getMyBakery, type DashboardStats, type Customer } from "@/lib/db";
...
function getUpcomingCustomers(customers: Customer[]) {
  ...
  customers.forEach((c) => {
    (["birthday", "anniversary"] as const).forEach((field) => {
...
type WAStatus = "connected" | "disconnected" | "checking";
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    async function load() {
      try {
        const [s, customers, bakery] = await Promise.all([
          getDashboardStats(),
          getCustomers(),
          getMyBakery(),
        ]);
        ...
        // Check real WhatsApp connection status from server
        if (bakery?.id) {
          try {
            const res = await fetch(`${SERVER_URL}/status/${bakery.id}`);
            if (res.ok) {
            ...
            // Server unreachable — show disconnected
            setWaStatus("disconnected");
          }
        } else {
          setWaStatus("disconnected");
    ...
    <AppLayout>
      <div className="max-w-7xl mx-auto">
        ...
        >
          ...
          {waStatus === "checking" ? (
            ...
            <div className="flex items-center gap-2 bg-red-50 border border-red-200 rounded-full px-3 py-1.5">
              ...
              <span className="text-xs font-medium text-red-600">WhatsApp Not Connected</span>
            </div>
          )}
        </motion.div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4 mb-8">
          ...
          <StatCard label="Messages Delivered"     value={deliveryRate}                                      icon={CheckCircle2} suffix="%" color="bg-green-100 text-green-600" delay={0.24} />
        </div>
          ...
          >
            <div className="flex items-center justify-between mb-6">
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-0.5">Messages sent vs delivered this month</p>
              </div>
            ...
            {stats && stats.sentThisMonth > 0 ? (
              <ResponsiveContainer width="100%" height={220}>
                <BarChart
                  data={[{ name: "This month", sent: stats.sentThisMonth, delivered: stats.deliveredThisMonth }]}
                  barGap={4} barSize={14}
                >
          ...
          >
            ...
            {loading ? (
              <div className="flex flex-col gap-3">
                {[1, 2, 3].map((i) => <div key={i} className="h-10 bg-muted/40 rounded-xl animate-pulse" />)}
              </div>
          ...
          {loading ? (
            ...
            <div className="flex flex-col gap-3">
              {upcomingReminders.map((reminder) => (
                <div key={reminder.id} className="flex items-center gap-4 p-3 rounded-xl bg-muted/40 hover:bg-muted/70 transition-colors">
                  <div className="w-10 h-10 rounded-full bg-primary/10 flex items-center justify-center shrink-0">
                    {reminder.occasion === "Birthday"
                  ...
                  <Badge variant="secondary" className={`text-xs shrink-0 ${reminder.daysUntil <= 5 ? "bg-orange-100 text-orange-700 border-orange-200" : ""}`}>
